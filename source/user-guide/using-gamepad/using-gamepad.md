...
2. 解锁
    ...
    在手柄开机并与PM1配对状态下，同时按下 `LB`、`RB`、`LT`、`RT` 四个按键，将解锁PM1，此时可以通过手柄/上位机控制PM1运动。
...
2. 速度等级（示例中按下的防误触按键为`LB`）

    使用手柄遥控PM1时提供了5个速度等级（1-5），每次PM1开机后默认速度等级为1。

    按住防误触按键的前提下，按下手柄上的 `START` 键表示速度等级+1（最大为5），按下手柄上的`BACK`键表示速度等级-1（最小为1）。

    ![](imgs/gamepad_speed_level.gif)

3. 运动方向（示例中按下的防误触按键为`LB`）

    按住防误触按键的前提下，左侧摇杆、左侧方向板、右侧摇杆、右侧ABXY 均可用来控制PM1的运动方向。
